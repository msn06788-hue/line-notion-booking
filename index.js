...
async function checkDayBookings(replyToken, date, m, p, x) {
  try {
    ...
    console.error("Notion 查詢錯誤:", JSON.stringify(err, null, 2));
...
async function finalizeBooking(event, data) {
  ...
  try {
    ...
    await client.replyMessage(event.replyToken, [{ type: 'text', text: `✅ 預約成功！\n時段：${d} ${timeStr}\n總額：NT$ ${amount}` }, { type: 'text', text: SERVICE_INFO.staff + "\n" + SERVICE_INFO.phone + "\n" + SERVICE_INFO.closing }]);
  } catch (err) {
    console.error("Notion 寫入錯誤:", JSON.stringify(err, null, 2));
    await client.replyMessage(event.replyToken, { type: 'text', text: "❌ 預約失敗，請檢查 Notion 欄位名稱是否與程式碼對應。" });
  }
}
