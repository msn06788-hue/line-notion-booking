...
async function checkNotionBookings(replyToken, date, m, p, x) {
  try {
    ...
    // 針對 Notion 的「日期」屬性（Date 類型）進行查詢
    ...
    await client.replyMessage(replyToken, [
      { type: "text", text: info },
      { type: "text", text: "請滑動下方選取「起始時間」：", quickReply: { items: qrItems } }
    ]);
  } catch (err) { console.error("Notion 查詢錯誤:", err); }
}
...
async function finalizeBooking(event, data) {
  ...
  try {
    ...
    await client.replyMessage(event.replyToken, { type: 'text', text: "❌ 寫入失敗，請檢查 Notion 欄位名稱是否正確。" });
  }
}
