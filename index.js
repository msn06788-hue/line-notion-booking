...
const SERVICE_INFO = {
  ...
  bank: "🏦 匯款資訊：\n星展銀行 810 世貿分行\n帳號：602-489-60988\n戶名：鍾沛潔",
  closing: "\n\n感謝您的預訂！😊\n⚠️ 提醒：目前報價僅保留三天，匯款完成後請記得告知小編您的「帳號後五碼」喔！"
};
...
/**
 * 查詢 Notion 並顯示當日已訂時間 + 起始時間 Quick Reply
 */
async function checkDayBookings(replyToken, date, m, p, x) {
  try {
    ...
    let bookedStr = res.results.map(page => page.properties["時間"].rich_text[0].plain_text.split(" ")[1]).join(", ");
    let infoText = bookedStr ? `📅 ${date} 目前已被預訂：\n${bookedStr}` : `📅 ${date} 目前尚無預訂。`;
    ...
    const qrItems = times.map(time => ({
      type: "action", action: { type: "postback", label: time, data: `act=h_start&m=${m}&p=${p}&x=${x}&d=${date}&t=${time}` }
    }));

    await client.replyMessage(replyToken, [
      { type: "text", text: infoText },
      { type: "text", text: "請點選下方的「起始時間」：", quickReply: { items: qrItems } }
    ]);
...
async function finalizeBooking(event, data) {
  ...
  // 1. 自動計算結束時間 (t 為 HH:mm)
  let [startH, startM] = t.split(':').map(Number);
  let endH = startH + h;
  let displayTime = `${d} ${t}-${String(endH).padStart(2, '0')}:${String(startM).padStart(2, '0')}`;

  // 2. 判定時段單價 (根據起始小時判定早/午/晚)
  let slotKey = 'm'; 
  ...
  const totalAmount = (m === 'p' ? (isWknd ? pricing.p_we : pricing.p_wd) : (isWknd ? pricing.h_we : pricing.h_wd) * h);

  try {
    const dbId = getCleanDbId();
    // 防撞檢查
    const conflict = await notion.databases.query({
      database_id: dbId, filter: { property: "時間", rich_text: { equals: displayTime } }
    });

    if (conflict.results.length > 0) {
      return await client.replyMessage(event.replyToken, [
        { type: "text", text: `⚠️ 預約失敗！時段 ${displayTime} 已有人佔用。` },
        { type: "template", altText: "重選日期", template: { type: "buttons", title: "請更換日期", text: "請點選按鈕重新選取日期：", actions: [{ type: "datetimepicker", label: "📅 重選日期", data: `act=date&m=${m}&p=${p}&x=${x}`, mode: "date" }] } }
      ]);
    }
...
async function sendPriceList(replyToken) {
  return await client.replyMessage(replyToken, [{ type: 'image', originalContentUrl: "https://raw.githubusercontent.com/msn06788-hue/line-notion-booking/main/price_list.png", previewImageUrl: "https://raw.githubusercontent.com/msn06788-hue/line-notion-booking/main/price_list.png" }]);
}
